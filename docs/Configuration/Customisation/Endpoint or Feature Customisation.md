...
- The [MCP server](../Automation/MCP-Server.md) applies its own allow/deny filtering on top of these removals, so an endpoint can be enabled here yet still be hidden from MCP clients
